package com.example.library.views.register;
    ...
    public Register(UserService userService,EmailService emailService) {
       ...
       userEmail.add(username,email);
        ...
        binder.forField(username).
                withValidator(v -> userService.findAll().stream().noneMatch(p -> p.getUsername().equals(v)), "\n" +
                        "Podana nazwa użytkownika już istnieje")
                .withValidator(v -> v.length() >=4 && v.length()<100, "Size min=4, max=100")
                .bind(User::getUsername, User::setUsername);

        binder.forField(email).
                withValidator(v -> userService.findAll().stream().noneMatch(p -> p.getEmail().equals(v)), "\n" +

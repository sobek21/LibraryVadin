...
import com.example.library.domain.bookSearch.BookFact;
    ...
    public VolumeInfo getBooks(String bookTitle) {
        ...
        ResponseEntity<BookFact> bookFact = restTemplate.exchange(
                "https://www.googleapis.com/books/v1/volumes?q=" +
                        bookTitle + "&startIndex=1&maxResults=1",
                HttpMethod.GET,
                HttpEntity.EMPTY,
                BookFact.class);


        VolumeInfo volumeInfo = bookFact.getBody().getItems().stream().findFirst().get().
                getVolumeInfo();
